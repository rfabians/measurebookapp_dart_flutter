...
class EditPoint extends StatefulWidget {
  ...
  final String nombreProyecto, proyeccion, idsistemaCoor;
  final puntosReferencia puntoRef;
  EditPoint({Key key, this.nombreProyecto, this.proyeccion, this.idUserMB, this.idsistemaCoor, this.puntoRef}) : super(key: key);
...
class _EditPointState extends State<EditPoint> {
  ...
  Widget wFotoOeste() {
    ...
    Image fotoPlacaF () {
      if (widget.puntoRef.Foto_Placa == null) {
        return null;
      } else {
        return Image.memory(base64Decode(widget.puntoRef.Foto_Placa));
      }
    }
    Image fotoNorteF () {
      if (widget.puntoRef.Foto_Norte == null) {
        return null;
      } else {
        return Image.memory(base64Decode(widget.puntoRef.Foto_Norte));
      }
    }
    Image fotoEsteF () {
      if (widget.puntoRef.Foto_Este == null) {
        return null;
      } else {
        return Image.memory(base64Decode(widget.puntoRef.Foto_Este));
      }
    }
    Image fotoSurF () {
      if (widget.puntoRef.Foto_Sur == null) {
        return null;
      } else {
        return Image.memory(base64Decode(widget.puntoRef.Foto_Sur));
      }
    }
    Image fotoOesteF () {
      if (widget.puntoRef.Foto_Oeste == null) {
        return null;
      } else {
        return Image.memory(base64Decode(widget.puntoRef.Foto_Oeste));
      }
    }
  ...
  void initState() { 
    super.initState();
    fotoPlaca = fotoPlacaF();
    fotoNorte = fotoNorteF();
    fotoEste = fotoEsteF();
    fotoSur = fotoSurF();
    fotoOeste = fotoOesteF();
                    ...
                    FlatButton(onPressed: () {
                      if (_formKey.currentState.validate()) {
                          ...
                          builder: (context) => GestosPuntos(
                            idProyeccion: widget.idsistemaCoor,
