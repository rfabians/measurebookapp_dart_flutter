...
import 'package:measurebookapp/pages/observacionesGNSS/descripcionVerticeII.dart';
...
class _DescripcionVerticeState extends State<DescripcionVertice>  {
  ...
  String fechaMonumentacion, tipoMonumentacion, anchoMonumentacion, largoMonumentacion, sobresaleMonumentacion, estadoMonumento, monumentadoPor;
                 ...
                 child: Center(
                   child: Column(
                      children: <Widget>[
                       ...
                       Divider(),
                       Text('Datos de Monumentación', style: TextStyle(fontSize: 12, color: Colors.black54)),
                       Divider(),
                       DateTimeField(
                         ...
                         validator: (fechaMonumen){
                          if(fechaMonumen == null){
                            ...
                            setState(() {
                              fechaMonumentacion = DateFormat('yyyy-MM-dd').format(fechaMonumen);
                            });
                          }
                         },
                         ),
                        TextFormField(
                         decoration: InputDecoration(
                           labelStyle: TextStyle(fontSize: 12),
                           icon: Icon(Icons.all_out, color: Colors.blueAccent),
                       ...
                       GestureDetector(
                         onTap: (){
                            if(_monumentacion.currentState.validate()){
                             Navigator.push(context, MaterialPageRoute(
                               builder: (context) => DescripcionVerticeII()                            
                             ));
                           }
                         },
                         child: Column(
                           children: <Widget>[
                             SizedBox(height: 10),
                             Icon(Icons.navigate_next, size: 70, color: Colors.blueAccent),
                             Text('Siguiente', style: TextStyle(color: Colors.black54, fontSize: 12)),
                           ],
                         ),
                       )
                      ],
                   ...
                   child: Column(
                     children: <Widget>[
                       ...
                       DateTimeField(
                         ...
                         validator: (fecha){
                          if(fecha == null){
                            ...
                            setState(() {
                              fechaS = DateFormat('yyyy-MM-dd').format(fecha);
                            });
                          }
                         },
                         ),
                      ...
                      DropDownField(
                        ...
                        labelStyle:  TextStyle(fontSize: 12, color: Colors.black54, fontStyle: FontStyle.normal),
                        hintText: "Seleccione un Departamento",
                        hintStyle: TextStyle(fontSize: 12, color: Colors.black54, fontStyle: FontStyle.normal),
                        enabled: true,
                        items: listadoDepartamentos,
                        icon: Icon(Icons.map, color: Colors.blueAccent),
                        textStyle: TextStyle(fontSize: 12, color: Colors.black54, fontStyle: FontStyle.normal),
                        onValueChanged: (departamentoSelect ) async {
                            controladorMunicipio.clear();
                            int pkDep = departamentosMap[departamentoSelect];
                            List<String> listaNombreMunicipios = List<String>();
                            List<municipios> municipiosDB = await gestorMBDatabase.db.getMunicipios(pkDep);
                            for (var i = 0; i < municipiosDB.length; i++) {
                              listaNombreMunicipios.add(municipiosDB[i].NOMBRE);
                            }
                          setState(() {
                      ...
                      Padding(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: Colors.white,
                            boxShadow: [BoxShadow(color: Colors.black87, blurRadius: 2)]
                          ),
                          child: Center(
                            child: Padding(
                              ...
                              child: SingleChildScrollView(
                                ...
                                child: Column(
                                  children: [
                                    SizedBox(height: 5),
                                    Text('Coordenadas Navegadas de Punto MAGNA SIRGAS', style: TextStyle(fontSize: 12, color: Colors.blueAccent)),
                                    Divider(),
                                    Row(
                                    ...
                                    children: <Widget>[
                                      Column(
                                        children: <Widget>[
                                          Text('Latitud', style: TextStyle(fontSize: 12, color: Colors.blueAccent)),
                                          Container(
                                            width: 100,
                                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(15), color: Colors.black45),
                                            child: Padding(
                                            padding: const EdgeInsets.all(8.0),
                                            child: Text('${latitudPUnto}', style: TextStyle(fontSize: 12, color: Colors.white),textAlign: TextAlign.center),
                                          ))
                                        ],
                                      ),
                                      SizedBox(width: 10),
                                      Column(
                                        children: <Widget>[
                                          Text('Longitud', style: TextStyle(fontSize: 12, color: Colors.blueAccent),textAlign: TextAlign.center),
                                          Container(
                                            width: 100,
                                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(15), color: Colors.black45),
                                            child: Padding(
                                            padding: const EdgeInsets.all(8.0),
                                            child: Text('${longitudPunto}', style: TextStyle(fontSize: 12, color: Colors.white),textAlign: TextAlign.center),
                                          ))
                                        ],
                                      ),
                                      SizedBox(width: 10),
                                      Column(
                                        children: <Widget>[
                                          Text('Altura', style: TextStyle(fontSize: 12, color: Colors.blueAccent),textAlign: TextAlign.center),
                                          Container(
                                            width: 100,
                                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(15), color: Colors.black45),
                                            child: Padding(
                                            padding: const EdgeInsets.all(8.0),
                                            child: Text('${alturaPuntos}', style: TextStyle(fontSize: 12, color: Colors.white),textAlign: TextAlign.center),
                                          ))
                                        ],
                                      )
                                    ],
                                  ...
                                  Row(
                                    children: <Widget>[
                                      ...
                                      child: GestureDetector(
                                        onTap: ()async{
                                          Position posicionUser = await Geolocator().getCurrentPosition();
                                          setState(() {
                                            latitudPUnto = funcionesGenericas.decimal2Hexadecimal(posicionUser.latitude);
                                            longitudPunto = funcionesGenericas.decimal2Hexadecimal(posicionUser.longitude);
                                            alturaPuntos = funcionesGenericas.redondearDouble(posicionUser.altitude, 3).toString();
                                          });
                                        },
                                        child: Text('Obtener coordenadas', style: TextStyle(fontSize: 12, color: Colors.white), textAlign: TextAlign.center)),
                                    )),
                                    SizedBox(width: 10),
                                    Container(
                                      width: 130,
                                      ...
                                      child: GestureDetector(
                                        onTap: (){
                                          AlertDialog ingresarCoordenadasManuales = AlertDialog(
                                            ...
                                            content: Container(
                                              ...
                                              child: Column(
                                                children: <Widget>[
                                                  Icon(Icons.location_searching, color: Colors.blueAccent, size: 40),
                                                  Text('Ingresar las coordenadas Manuales de tu Punto navegado', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: Colors.black54)),
                                                  Form(
                                                    key: _formElip,
                                                    child: Column(
                                                      children: <Widget>[
                                        ...
                                        child: Text('Ingresar Coordenadas', style: TextStyle(fontSize: 12, color: Colors.white), textAlign: TextAlign.center)),
                                    ))
                                    ],
                                  )
                      ...
                      Center(
                        child: Column(
                          children: <Widget>[
                            SizedBox(height: 10),
                            GestureDetector(
                              child: Icon(Icons.navigate_next, size: 60, color: Colors.blueAccent),
                              onTap: (){
                            ...
                            Text('Continuar', style: TextStyle(fontSize: 12, color: Colors.black54)),
                          ],
                        ),
                      )
                     ],
